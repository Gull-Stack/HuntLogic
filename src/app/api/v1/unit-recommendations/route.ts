...
export async function POST(request: NextRequest) {
  try {
    ...
    // Build success rate map: unit -> most recent success rate
    const successMap = new Map<string, number>();
    for (const row of harvestRows) {
      if (row.huntUnitId && row.successRate !== null && !successMap.has(row.huntUnitId)) {
    ...
    // -- Assign tiers and generate recommendations --
    const results: UnitRecommendationResult[] = topUnits.map((unit, idx) => {
...
function generateRecommendation(
  ...
  if (motivation === "meat") {
    if (unit.canDrawNow && srPct !== null && srPct >= 40) {
      return `Strong meat-hunter pick with ${srPct}% success rate and realistic draw odds.`;
    }
    if (srPct !== null && srPct >= 30) {
      return `Solid success rate (${srPct}%) makes this a reliable harvest unit.`;
    }
    return `Worth considering for a meat hunt${drPct !== null ? ` with ${drPct}% draw odds` : ""}.`;
  }

  if (motivation === "trophy" || motivation === "otl") {
    if (unit.canDrawNow && tier === "A") {
      return `Top-tier trophy unit you can draw now with ${userPoints} points${srPct ? ` and ${srPct}% success` : ""}.`;
    }
    if (!unit.canDrawNow && minPts > 0) {
      const deficit = minPts - userPoints;
      return `Premium unit requiring ~${deficit} more point${deficit === 1 ? "" : "s"} to draw — high demand for a reason.`;
    }
    return `High-quality unit${drPct !== null ? ` with ${drPct}% draw odds` : ""} worth your consideration.`;
  }

  // balanced
  if (unit.canDrawNow && srPct !== null && srPct >= 30) {
    return `Well-rounded unit: drawable now with ${srPct}% success and ${drPct ?? "N/A"}% odds.`;
  }
  return `Balanced pick${drPct !== null ? ` at ${drPct}% draw odds` : ""}${srPct !== null ? ` and ${srPct}% success` : ""}.`;
}
